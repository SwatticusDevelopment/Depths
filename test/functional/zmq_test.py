...
from test_framework.mininode import Depthshash
...
def Depthshash_helper(b):
    return encode(Depthshash(b)[::-1], 'hex_codec').decode('ascii')
...
class ZMQTest (BitcoinTestFramework):
    ...
    def setup_nodes(self):
        ...
        # Check that Depths has been built with ZMQ enabled
    ...
    def _zmq_test(self):
        ...
        assert_equal(genhashes[0], Depthshash_helper(body[:80]))
        ...
        for x in range(n * 4):
            ...
            if topic == b"hashblock":
                ...
                blockcount += 1
            if topic == b"rawblock":
                zmqRawHashed.append(Depthshash_helper(body[:80]))
                msgSequence = struct.unpack('<I', msg[-1])[-1]
