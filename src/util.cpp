...
const char * const BITCOIN_CONF_FILENAME = "Depths.conf";
const char * const BITCOIN_PID_FILENAME = "Depthsd.pid";
...
{
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\DepthsCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\DepthsCore
    // Mac: ~/Library/Application Support/DepthsCore
    // Unix: ~/.Depthscore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "DepthsCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/DepthsCore";
#else
    // Unix
    return pathRet / ".Depthscore";
#endif
#endif
}
...
void ArgsManager::ReadConfigFile(const std::string& confPath)
{
    ...
    if (!streamConfig.good()){
        // Create empty Depths.conf if it does not excist
...

    {
        ...
        {
            // Don't overwrite existing settings so command line settings override Depths.conf
