...
// which are part of the Depths repo will allow modification to the Jenkinsfile.
...
for(int i = 0; i < targets.size(); i++) {
  ...
  tasks["${target}"] = {
    node {
      ...
      withEnv(env) {
        def builderImageName="Depths-builder-${target}"
        ...
        builderImage.inside("-t") {
          ...
          sh "cp -ra ${pwd}/. /Depths-src/"

          // restore cache
          def hasCache = false
          try {
            copyArtifacts(projectName: "Depths-Depths/${BRANCH_NAME}", optional: true, selector: lastSuccessful(), filter: "ci-cache-${target}.tar.gz")
          } catch (Exception e) {
          }
          if (fileExists("ci-cache-${target}.tar.gz")) {
            hasCache = true
            echo "Using cache from Depths-Depths/${BRANCH_NAME}"
          } else {
            try {
              copyArtifacts(projectName: 'Depths-Depths/develop', optional: true, selector: lastSuccessful(), filter: "ci-cache-${target}.tar.gz");
            } catch (Exception e) {
            }
            if (fileExists("ci-cache-${target}.tar.gz")) {
              hasCache = true
              echo "Using cache from Depths-Depths/develop"
            }
          }

          if (hasCache) {
            sh "cd /Depths-src && tar xzf ${pwd}/ci-cache-${target}.tar.gz"
          } else {
            sh "mkdir -p /Depths-src/ci-cache-${target}"
          }

          stage("${target}/depends") {
            sh 'cd /Depths-src && ./ci/build_depends.sh'
          }
          stage("${target}/build") {
            sh 'cd /Depths-src && ./ci/build_src.sh'
          }
          stage("${target}/test") {
            sh 'cd /Depths-src && ./ci/test_unittests.sh'
          }
          stage("${target}/test") {
            sh 'cd /Depths-src && ./ci/test_integrationtests.sh'
          }

          // archive cache and copy it into the jenkins workspace
          sh "cd /Depths-src && tar czfv ci-cache-${target}.tar.gz ci-cache-${target} && cp ci-cache-${target}.tar.gz ${pwd}/"
        }
