...
PACKAGE_VERSION=$($SRC_PATH/src/Depthsd --version | grep version | cut -d' ' -f4 | tr -d v)
...
# Copy binaries
cp $SRC_PATH/src/Depthsd $DEB_BIN
cp $SRC_PATH/src/Depths-cli $DEB_BIN
cp $SRC_PATH/util/fetch-params.sh $DEB_BIN/Depths-fetch-params
# Copy docs
...
# Copy manpages
cp $SRC_DOC/man/Depthsd.1 $DEB_MAN
cp $SRC_DOC/man/Depths-cli.1 $DEB_MAN
cp $SRC_DOC/man/Depths-fetch-params.1 $DEB_MAN
# Copy bash completion files
cp $SRC_PATH/contrib/Depthsd.bash-completion $DEB_CMP/Depthsd
cp $SRC_PATH/contrib/Depths-cli.bash-completion $DEB_CMP/Depths-cli
# Gzip files
gzip --best -n $DEB_DOC/changelog
gzip --best -n $DEB_DOC/changelog.Debian
gzip --best -n $DEB_MAN/Depthsd.1
gzip --best -n $DEB_MAN/Depths-cli.1
gzip --best -n $DEB_MAN/Depths-fetch-params.1

cd $SRC_PATH/contrib

# Create the control file
dpkg-shlibdeps $DEB_BIN/Depthsd $DEB_BIN/Depths-cli
