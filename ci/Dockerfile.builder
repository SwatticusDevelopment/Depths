...
# Depths_hash
RUN git clone https://github.com/The-Depths-Endeavor/Depths_hash
RUN cd Depths_hash && python3 setup.py install
...
RUN groupadd -g ${GROUP_ID} Depths
RUN useradd -u ${USER_ID} -g Depths -s /bin/bash -m -d /Depths Depths
...
RUN mkdir /Depths-src && \
  mkdir -p /cache/ccache && \
  mkdir /cache/depends && \
  mkdir /cache/sdk-sources && \
  chown $USER_ID:$GROUP_ID /Depths-src && \
